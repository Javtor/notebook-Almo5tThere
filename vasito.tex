...
$C_n$ is the number of different ways $n+1$ factors can be completely parenthesized ${(((ab)c)d),(a(bc))d,...}$. Ex: number of full binary trees $n+1$ leaves
...
$S_{n,k}$ Formas de partir un conjunto de $n$ elementos en $k$ conjuntos
$\forall n\geq1S_{n,1}=1, \forall n>1 S_{1,k}=0, S_{n,k}=k*S_{n-1,k}+S_{n-1,k-1}$
...
\subsection{Floyd cycle finding algorithm}
\cppfile{other/floydcycle.cpp}
\subsection{Dates}
